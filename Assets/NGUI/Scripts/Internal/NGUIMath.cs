...
static public class NGUIMath
{
	...
	static public Bounds CalculateAbsoluteWidgetBounds (Transform trans)
	{
		if (trans != null)
		{
			View[] widgets = trans.GetComponentsInChildren<View>() as View[];
		...
		ref Matrix4x4 toLocal, ref Vector3 vMin, ref Vector3 vMax, ref bool isSet, bool considerChildren)
	{
		...
		else // No panel present
		{
			...
			View w = content.GetComponent<View>();
	...
	static public Vector2 GetPivotOffset (View.Pivot pv)
	{
		Vector2 v = Vector2.zero;

		if (pv == View.Pivot.Top || pv == View.Pivot.Center || pv == View.Pivot.Bottom) v.x = 0.5f;
		else if (pv == View.Pivot.TopRight || pv == View.Pivot.Right || pv == View.Pivot.BottomRight) v.x = 1f;
		else v.x = 0f;

		if (pv == View.Pivot.Left || pv == View.Pivot.Center || pv == View.Pivot.Right) v.y = 0.5f;
		else if (pv == View.Pivot.TopLeft || pv == View.Pivot.Top || pv == View.Pivot.TopRight) v.y = 1f;
		else v.y = 0f;
	...
	static public View.Pivot GetPivot (Vector2 offset)
	{
		if (offset.x == 0f)
		{
			if (offset.y == 0f) return View.Pivot.BottomLeft;
			if (offset.y == 1f) return View.Pivot.TopLeft;
			return View.Pivot.Left;
		}
		else if (offset.x == 1f)
		{
			if (offset.y == 0f) return View.Pivot.BottomRight;
			if (offset.y == 1f) return View.Pivot.TopRight;
			return View.Pivot.Right;
		}
		else
		{
			if (offset.y == 0f) return View.Pivot.Bottom;
			if (offset.y == 1f) return View.Pivot.Top;
	...
	static public void ResizeWidget (View w, View.Pivot pivot, float x, float y, int minWidth, int minHeight)
	{
	...
	static public void ResizeWidget (View w, View.Pivot pivot, float x, float y, int minWidth, int minHeight, int maxWidth, int maxHeight)
	{
		if (pivot == View.Pivot.Center)
		{
	...
	static public void AdjustWidget (View w, float left, float bottom, float right, float top)
	{
	...
	static public void AdjustWidget (View w, float left, float bottom, float right, float top, int minWidth, int minHeight)
	{
