...
static public class NGUIEditorTools
{
	...
	static public List<View> SceneViewRaycast (Vector2 mousePos)
	{
		...
		list.Sort(View.FullCompareFunc);
		return list;
	}
	...
	{
		GameObject go = null;
		List<View> widgets = SceneViewRaycast(pos);
 ...
 #endif
			View.showHandlesWithMoveTool && !NGUISettings.showTransformHandles;
#endif
	}
	...
	static public void UpgradeTexturesToSprites (UIAtlas atlas)
	{
		...
		if (uits.Count > 0)
		{
			View selectedTex = (UIWidgetInspector.instance != null && UIWidgetInspector.instance.target != null) ?
	...
	{
		List<View> widgets = NGUIEditorTools.SceneViewRaycast(screenPos);
