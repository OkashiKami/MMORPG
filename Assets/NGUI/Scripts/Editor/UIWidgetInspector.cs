...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public View.Pivot[] pivotPoints =
	{
		View.Pivot.BottomLeft,
		View.Pivot.TopLeft,
		View.Pivot.TopRight,
		View.Pivot.BottomRight,
		View.Pivot.Left,
		View.Pivot.Top,
		View.Pivot.Right,
		View.Pivot.Bottom,
	...
	View.Pivot mDragPivot = View.Pivot.Center;
	...
	static public View.Pivot GetPivotUnderMouse (Vector3[] worldPos, Event e, bool[] resizable, bool movable, ref Action action)
	{
		// Time to figure out what kind of action is underneath the mouse
		View.Pivot pivotUnderMouse = View.Pivot.Center;
	...
	public void OnSceneGUI ()
	{
		...
		if (!View.showHandles) return;

		mWidget = target as View;
		...
		if (mWidget.keepAspectRatio == View.AspectRatioSource.BasedOnHeight)
		{
			resizable[4] = false;
			resizable[6] = false;
		}
		else if (mWidget.keepAspectRatio == View.AspectRatioSource.BasedOnWidth)
		{
			resizable[5] = false;
			resizable[7] = false;
		}
		...
		View.Pivot pivotUnderMouse = GetPivotUnderMouse(handles, e, resizable, true, ref actionUnderMouse);
		...
		switch (type)
		{
			...
			case EventType.MouseUp:
			{
				...
				{
					List<View> widgets = NGUIEditorTools.SceneViewRaycast(e.mousePosition);
	...
	static public void DrawColor (SerializedObject so, View w)
	{
		if ((w.GetType() != typeof(View)))
		{
	...
	static public void DrawInspectorProperties (SerializedObject so, View w, bool drawColor)
	{
		if (drawColor)
		{
			DrawColor(so, w);
	...
	static void DrawDimensions (SerializedObject so, View w, bool isPrefab)
	{
		GUILayout.BeginHorizontal();
		{
	...
	static void DrawDepth (SerializedObject so, View w, bool isPrefab)
	{
		if (isPrefab) return;
	...
	static void DrawPivot (SerializedObject so, View w)
	{
		...
		else
		{
			...
			Toggle(w, "\u25C4", "ButtonLeft", View.Pivot.Left, true);
			Toggle(w, "\u25AC", "ButtonMid", View.Pivot.Center, true);
			Toggle(w, "\u25BA", "ButtonRight", View.Pivot.Right, true);
			Toggle(w, "\u25B2", "ButtonLeft", View.Pivot.Top, false);
			Toggle(w, "\u258C", "ButtonMid", View.Pivot.Center, false);
			Toggle(w, "\u25BC", "ButtonRight", View.Pivot.Bottom, false);
	...
	static void Toggle (View w, string text, string style, View.Pivot pivot, bool isHorizontal)
	{
		bool isActive = false;

		switch (pivot)
		{
			case View.Pivot.Left:
			isActive = IsLeft(w.pivot);
			break;

			case View.Pivot.Right:
			...
			case View.Pivot.Bottom:
			isActive = IsBottom(w.pivot);
			break;

			case View.Pivot.Center:
	...
	static bool IsLeft (View.Pivot pivot)
	{
		return pivot == View.Pivot.Left ||
			pivot == View.Pivot.TopLeft ||
			pivot == View.Pivot.BottomLeft;
	}

	static bool IsRight (View.Pivot pivot)
	{
		return pivot == View.Pivot.Right ||
			pivot == View.Pivot.TopRight ||
			pivot == View.Pivot.BottomRight;
	}

	static bool IsTop (View.Pivot pivot)
	{
		return pivot == View.Pivot.Top ||
			pivot == View.Pivot.TopLeft ||
			pivot == View.Pivot.TopRight;
	}

	static bool IsBottom (View.Pivot pivot)
	{
		return pivot == View.Pivot.Bottom ||
			pivot == View.Pivot.BottomLeft ||
			pivot == View.Pivot.BottomRight;
	}

	static View.Pivot GetHorizontal (View.Pivot pivot)
	{
		if (IsLeft(pivot)) return View.Pivot.Left;
		if (IsRight(pivot)) return View.Pivot.Right;
		return View.Pivot.Center;
	}

	static View.Pivot GetVertical (View.Pivot pivot)
	{
		if (IsTop(pivot)) return View.Pivot.Top;
		if (IsBottom(pivot)) return View.Pivot.Bottom;
		return View.Pivot.Center;
	}

	static View.Pivot Combine (View.Pivot horizontal, View.Pivot vertical)
	{
		if (horizontal == View.Pivot.Left)
		{
			if (vertical == View.Pivot.Top) return View.Pivot.TopLeft;
			if (vertical == View.Pivot.Bottom) return View.Pivot.BottomLeft;
			return View.Pivot.Left;
		}

		if (horizontal == View.Pivot.Right)
		{
			if (vertical == View.Pivot.Top) return View.Pivot.TopRight;
			if (vertical == View.Pivot.Bottom) return View.Pivot.BottomRight;
			return View.Pivot.Right;
		}
		return vertical;
	}

	static void SetPivot (View w, View.Pivot pivot, bool isHorizontal)
	{
		View.Pivot horizontal = GetHorizontal(w.pivot);
		View.Pivot vertical = GetVertical(w.pivot);
