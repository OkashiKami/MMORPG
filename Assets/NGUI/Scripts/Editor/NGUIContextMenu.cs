...
static public class NGUIContextMenu
{
	...
	static void CopyStyle (MenuCommand command) { NGUISettings.CopyWidget(command.context as View); }

	[MenuItem("CONTEXT/UIWidget/Paste Widget Values")]
	static void PasteStyle (MenuCommand command) { NGUISettings.PasteWidget(command.context as View, true); }

	[MenuItem("CONTEXT/UIWidget/Paste Widget Style")]
	static void PasteStyle2 (MenuCommand command) { NGUISettings.PasteWidget(command.context as View, false); }
	...
	public delegate View AddFunc (GameObject go);
	...
	static public void AddChild (object obj)
	{
		...
		View widget = func(Selection.activeGameObject);
	...
	{
		AddFunc func = obj as AddFunc;
		View widget = func(Selection.activeTransform.parent.gameObject);
		if (widget != null) Selection.activeGameObject = widget.gameObject;
	}
	...
	static public void AddCommonItems (GameObject target)
	{
		if (target != null)
		{
			View widget = target.GetComponent<View>();
	...
	{
		GameObject go = obj as GameObject;
		if (go.GetComponent<View>() != null) go = go.transform.parent.gameObject;
