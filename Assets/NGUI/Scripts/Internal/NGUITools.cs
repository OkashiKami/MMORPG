...
//    }
...
#endif
				View widget = go.GetComponent<View>();
	...
	static public int CalculateRaycastDepth (GameObject go)
	{
		...
		var widgets = go.GetComponentsInChildren<View>();
#if UNITY_5_5_OR_NEWER
		UnityEngine.Profiling.Profiler.EndSample();
#else
		Profiler.EndSample();
	...
	static public int CalculateNextDepth (GameObject go)
	{
		...
		{
			int depth = -1;
			View[] widgets = go.GetComponentsInChildren<View>();
			for (int i = 0, imax = widgets.Length; i < imax; ++i)
	...
	static public int AdjustDepth (GameObject go, int adjustment)
	{
		if (go != null)
		{
			...
			else
			{
				...
				View[] widgets = go.GetComponentsInChildren<View>(true);
	...
	{
		NormalizeWidgetDepths(FindActive<View>());
	}
	...
	{
		NormalizeWidgetDepths(go.GetComponentsInChildren<View>());
	}
	...
	static public void NormalizeWidgetDepths (View[] list)
	{
		int size = list.Length;

		if (size > 0)
		{
			Array.Sort(list, View.FullCompareFunc);
	...
	static public T AddWidget<T> (this GameObject go, int depth = int.MaxValue) where T : View
	{
	...
	{
		ExecuteAll<View>(root, "Start");
		ExecuteAll<UIPanel>(root, "Start");
		ExecuteAll<View>(root, "Update");
...
//#endif
	...
	static Dictionary<string, View> mWidgets = new Dictionary<string, View>();
	static UIPanel mRoot;
	static GameObject mGo;

	public delegate void OnInitFunc<T> (T w) where T : View;
	...
	static public T Draw<T> (string id, OnInitFunc<T> onInit = null) where T : View
	{
		View w;
